class KalturaAd
  ...
  def buildOverlayAd ad
    ...
    output = {'ad_id' => ad.id, 'iqeinfo' => ad.iqeinfo_id, 'ads' => [ {'nonLinear'=> [ { 'width' => @ad_params[:width], 'height' => @ad_params[:height] + 5, 'html' => '<div style=”position:relative;background-color:' + @ad_params[:background_color] + ';font-size:' + @ad_params[:font_size] + ';opacity:' + @ad_params[:opacity].to_s + ';filter:alpha(opacity=' + @ad_params[:alpha].to_s + ');padding-left:' + @ad_params[:padding_left] + ';padding-right:' + @ad_params[:padding_right] + ';padding-top:' + @ad_params[:padding_top] + ';padding-bottom:' + @ad_params[:padding_bottom] + ';height:' + @ad_params[:height].to_s + 'px;width:' + @ad_params[:width].to_s + 'px"><a href="http://www.bettersense.com:3000/regclick?id=' + ad_id.to_s + '&url=' + ad_target + '" target="_blank" style="color:' + @ad_params[:a_color] + ';hover:' + @ad_params[:a_hover_color] + ':visited:' + @ad_params[:a_visited_color] + '"><span style="position:absolute;width:100%;height:100%;top:0;left:0"></span>' + overlay_content + '</a></div>'  } ] } ], 'start' => @ad_params[:start], 'timeout' => @ad_params[:timeout] }
    output
  end
